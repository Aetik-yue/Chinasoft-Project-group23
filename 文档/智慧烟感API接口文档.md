...
|---|---|
| 优先级 | P1 |
| 描述 | 账号密码登录（用户名或已绑定手机号 + 密码），返回 token 与用户资料。 |
| 鉴权 | 否 |
...
| 字段 | 类型 | 必填 | 说明 | 示例 |
|---|---|---|---|---|
| account | string | 是 | 用户名或已绑定的手机号。后端先按用户名查，查不到再按手机号查，因此绑定手机号后可直接用手机号 + 密码登录 | `admin` |
| password | string | 是 | 密码（演示阶段明文传输，正式环境应改为 BCrypt） | `123456` |
...
| measuredAt | datetime | 否 | 省略时 POST 由后端写入当前时间；PUT 保留原时间；显式传入时不得晚于当前时间 |
...
| capturedAt | datetime | 否 | 省略时由后端写入当前时间；显式传入时不得晚于当前时间 |
