...
|---|---|
| 优先级 | P1 |
| 描述 | 账号密码登录，返回 token。后端未做鉴权时可使用 mock 登录。 |
| 鉴权 | 否 |
...
| password | string | 是 | 密码（明文传输，HTTPS 下可接受；演示阶段） | `123456` |
...
**错误码**：`1001` 参数缺失、`2001` 账号或密码错误。

**备注**：前端校验非空后调用；成功后存 token 并跳转首页。
...
> 本模块已接入 MySQL。当前尚未实现登录鉴权，档案与账本的 `userId` 由后端固定为 `1`；相片接口只保存图片 URL 和元数据，不上传二进制文件。日期使用 `YYYY-MM-DD`，日期时间使用 ISO 8601 格式 `YYYY-MM-DDTHH:mm:ss`。
...
| 插入相片 | POST | `/parrots/{petId}/photos` | 后端生成 `MEDIA-UUID`，仅保存 URL |
| 删除相片 | DELETE | `/parrots/{petId}/photos/{mediaId}` | 物理删除相片元数据记录 |
...
```json
{
  ...
  "title": "监控截图",
  "fileUrl": "https://example.com/parrot/shot-001.png",
  "thumbnailUrl": "https://example.com/parrot/shot-001-thumb.png",
  "tags": "监控,站立",
  "cageId": null,
  "capturedAt": "2026-07-06T14:30:00"
}
```

`fileUrl`、`capturedAt` 必填，拍摄时间不得晚于当前时间。`mediaType` 只能为 `photo` 或 `screenshot`，默认 `photo`。删除成功响应包含 `mediaId`、`petId`、`deletedAt`；接口只删除数据库记录，不负责删除 URL 指向的文件。
...
| 相片记录 | 加载、插入、删除 | 操作 `pet_media_record` URL 元数据 | `/parrots/{petId}/photos` | 刷新相册 | 回滚删除并提示失败 |
